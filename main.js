...
const windowOption = require('./app/window')
...
if (!gotTheLock) {
    ...
    app.on('web-contents-created', (_, webContent) => {
        webContent.on("context-menu", (event, params) => contextmenu.contextMenu(event, params));
...
function createWindow() {
    ...
    mainWindow.webContents.setWindowOpenHandler(({ targetUrl, frameName, disposition, options, additionalFeatures }) => {
        // if (disposition === 'new-window') {
        return {
            action: "allow",
            overrideBrowserWindowOptions: windowOption.windowOption
        }
        // }
    })
...
function createMenu() {
    // Tray Icon
    tray = new Tray(windowOption.iconPath);
